...
This section shows k-means clustering\index{k-means clustering} of `iris` data (see \@ref(intro:iris) for details of the data). At first, we remove species from the data to cluster. After that, we apply function \func{kmeans()} to `iris2`, and store the clustering result in `kmeans.result`. The cluster number is set to 3 in the code below.
```{r, echo=FALSE}
set.seed(8953)
```
...
Functions \func{pam()} and \func{clara()} in package \pack{cluster}\cite{Maechler15cluster} are respectively implementations of PAM and CLARA in R. For both algorithms, a user has to specify $k$, the number of clusters to find. 
As an enhanced version of \func{pam()}, function \func{pamk()} in package \pack{fpc} \cite{Hennig15fpc} does not require a user to choose $k$. Instead, it calls the function \func{pam()} or \func{clara()} to perform a partitioning around medoids clustering with the number of clusters estimated by optimum average silhouette width. 

With the code below, we demonstrate how to find clusters with \func{pam()} and \func{pamk()}.
...
```{r cluster:pamk, fig.cap='Clustering with the $k$-medoids Algorithm - I', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r cluster:kmedoids2, fig.cap='Clustering with the $k$-medoids Algorithm - II', out.width='80%', fig.asp=.75, fig.align='center'}
layout(matrix(c(1,2),1,2)) # 2 graphs per page 
plot(pam.result)
layout(matrix(1)) # change back to one graph per page 
```

With the above result produced with \func{pam()}, there are three clusters: 1) cluster 1 is species ``setosa'' and is well separated from the other two; 2) cluster 2 is mainly composed of ``versicolor'', plus some cases from ``virginica''; and 3) the majority of cluster 3 are ``virginica'', with two cases from ``versicolor''. 

It's hard to say which one is better out of the above two clusterings produced respectively with \func{pamk()} and \func{pam()}. It depends on the target problem and domain knowledge and experience. In this example, the result of \func{pam()} seems better, because it identifies three clusters, corresponding to three species. Therefore, the heuristic way to identify the number of clusters in \func{pamk()} does not necessarily produce the best result. Note that we cheated by setting $k=3$ when using \func{pam()}, which is already known to us as the number of species.


More examples of $k$-medoids clustering can be found in \@ref(sec:tm:kmedoids).



## Hierarchical Clustering}
This section demonstrates hierarchical clustering \index{hierarchical clustering} with \func{hclust()} on `iris} data (see \@ref(intro:iris) for details of the data).
...
Similar to the above clustering of $k$-means, \@ref(fig:cluster:hierarchical) also shows that cluster ``setosa" can be easily separated from the other two clusters, and that clusters ``versicolor" and ``virginica" are to a small degree overlapped with each other.

More examples of hierarchical clustering can be found in \@ref(sec:ts-cluster) and \@ref(sec:tm:cluster-words).



## Density-based Clustering} {#clustering:DBSCAN}
The DBSCAN algorithm \cite{Ester96Density} from package \pack{fpc} \cite{Hennig15fpc} provides a density-based clustering \index{density-based clustering} for numeric data. The idea of density-based clustering is to group objects into one cluster if they are connected to one another by densely populated area. There are two key parameters in DBSCAN \index{DBSCAN}:
\begin{itemize}
  \item `eps:} reachability distance, which defines the size of neighborhood; and
  \item `MinPts:} minimum number of points. 
\end{itemize}
If the number of points in the neighborhood of point $\alpha$ is no less than `MinPts}, then $\alpha$ is a \textit{dense point}. All the points in its neighborhood are \textit{density-reachable} from $\alpha$ and are put into the same cluster as $\alpha$. 
...
```
In the above table, ``1'' to ``3'' in the first column are three identified clusters, while ``0'' stands for noises or outliers\index{outlier}, i.e., objects that are not assigned to any clusters. The noises are shown as black circles in \@ref(fig:clustering:density1).
...
As we can see from the above result, out of the 10 new unlabeled data, 8(=3+3+2) are assigned with correct class labels. The new data are shown as asterisk("*") in the above figure and the colors stand for cluster labels in \@ref(fig:clustering:prediction).
